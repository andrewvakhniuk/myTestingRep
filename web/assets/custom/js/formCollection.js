...
function FormCollection(div_id)
{
    ...
    self.construct = function () {
        ...
        // [].forEach.call(self.div.children(), function(child) {
        //     self.addDeleteLink(child);
        //
        //     [].forEach.call(child.children(), function(item) {
        //         self.addDeleteLink(item);
        //     });
        // });

        $('[id='+div_id+']').children().each(function() {
        ...
        $('[id='+div_id+']').each(function() {
            var addLink=$('<a href="#" class="add-collection-item"><p></p><button type="button" class="btn btn-default btn-sm ">add</button></a>');
     ...
     */
    self.addNew = function (currentDiv) {
        ...
        var newForm = prototype.replace(/__choiveItems__/g, index);

        // increase the index with one for the next item
        self.div.data('index', index + 1);
        var $newFormLi = $('<li></li>').append(newForm);
        // Display the form in the page in an li, before the "Add a tag" link li
        $(currentDiv).before($newFormLi);

        // add a delete link to the new form
        self.addDeleteLink( $newFormLi);
